...
<div class="person-form">
    ...
    <p>
        <?php
        $url = ['create','asDialog' => 1, 'heading' => 1];
        if ( !$model->isNewRecord ){
          $url = ['update','asDialog' => 1, 'heading' => 1];
        }
        ?>
        <?= Html::a(Yii::t('pedido', 'Add detail'), $url , ['class' => 'btn btn-flat btn-success add-detail']) ?>
    </p>
...
$this->registerJs("
  $( document ).ready( function( ){
    $( '.add-detail' ).on( 'click', function( e ){
      ...
      $.ajax( {
        'url'    : $( 'form#Pedido' ).attr( 'action' ),
        'method' : $( 'form#Pedido' ).attr( 'method' ),
        ...
        success: function ( data ){
          ...
          {
             if ( data.heading ){
               parent.$( parent.buttonSubmit_detail ).css( 'display', 'block' );
               parent.$( parent.buttonSubmit_detail ).css( 'float', 'right' );
               parent.$( parent.frame_detail ).attr( \"src\", '".Url::to(['pedido-detalle/create','asDialog' => 1])."&idPedido=' + data.id);
               parent.$( parent.modal_detail ).modal({
                 backdrop: 'static',
                 keyboard: false,
               });
               parent.$( parent.modal_detail ).modal(\"show\");
               parent.$.pjax.reload( { container: '#grid' } );
               ".
               (( $model->isNewRecord ) ?
                         "
                         let url = '".Url::to(['pedido/update','asDialog' => 1])."&id=' + data.id;
                         setTimeout(function(){ parent.$( parent.frame ).attr('src',url);}, 3000);
                         "
                         : "")."
               return false;
            }
          ...
          $( 'form#Pedido' ).yiiActiveForm( 'updateMessages', data);
          return false;
        },
        error: function(data) {
            let message;
            ...
            window.parent.swal('Oops!!!',message,\"error\" );
            return false;
        }
      });
    });
      ...
      /* let parent = window.parent;
        ...
        parent.$( parent.buttonSubmit_detail ).css( 'display', 'block' );
        parent.$( parent.buttonSubmit_detail ).css( 'float', 'right' );
        parent.$( parent.frame_detail ).attr( \"src\", $( this ).attr( 'href' ));
        parent.$( parent.modal_detail ).modal({
          backdrop: 'static',
          keyboard: false,
        });
